...
- In this course, we will only scratch the surface of hierarchical modeling. 

--

- If you want to explore hierarchical models in much more detail, specifically within the context of Bayesian modeling, consider taking STA 610.
...
- We will use data from the Nicholas School of the Environment's study of mercury concentrations in largemouth bass from the Waccamaw and Lumber rivers in NC. 
...
- Model comparison between two models works but not exactly the same. 

--

- We will not dive deeply into estimation but basically, REML produces unbiased estimates for the random effects, whereas ML produces unbiased estimates for the fixed effects.
  + When using REML, which is the default in R, you should keep the fixed effects part the same when comparing two models.
  + When using ML, you should keep the random effects part the same when comparing two models.
