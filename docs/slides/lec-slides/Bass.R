...
xyplot(mercury ~ length | station, data = bass)
...
mercreg = lm(mercury ~ length.c * station, data = bass)
...
mercregnoint = lm(mercury ~ length.c + station, data = bass)
...
mercreglmerint = lmer(mercury ~ length.c + (1 | station), data = bass) 
...
mercreglmerintslope = lmer(mercury ~ length.c + ( 1 + length.c  | station), data = bass) 
...
#also reasonable.  So, we feel good about hierarchical model.
