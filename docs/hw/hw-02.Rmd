...
date: "Due: 6:00pm, Tuesday, Sept 17"
output: 
  html_document: 
    css: hw.css
    theme: yeti
...
This assignment involves multiple linear regression. The data (for question 2) can be found on Sakai: go to Resources $\rightarrow$ Datasets $\rightarrow$ Homework Datasets $\rightarrow$ Methods and Data Analysis 2. Please type your solutions using R Markdown, LaTeX or any other word processor but be sure to knit or convert the final output file to ".pdf". Submissions should be made on gradescope: go to Assignments $\rightarrow$ Methods and Data Analysis 2.
...
Reconsider the question on the Old Faithful eruptions from the last homework. Remember the data inlcuded a variable for the day of the eruptions (called "Date" in the dataset).  
    *Use the same "OldFaithful.csv" file you used for the first homework.*
      ...
      + Compute the in-sample RMSE for this model and do the same for your model from the last homework. Which model appears to have higher (within-sample) predictive accuracy based on the RMSE values?
